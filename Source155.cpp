...
public:
    ...
    // Self-Expansion (Domain-Specific for Multi-System Buoyancy)
    ...
    void expandBuoyancyScale(const std::string& system, double buoyancy_factor, double vacuum_factor);
    
    // Self-Refinement
    void autoRefineParameters(double tolerance);
    void calibrateToObservations(const std::map<std::string, cdouble>& observations);
    void optimizeForMetric(const std::string& metric, const std::string& system);
    ...
    void evolveSystem(int generations, std::function<double(const UQFFBuoyancyModule&)> fitness);
    ...
    // System Analysis
    std::map<std::string, double> sensitivityAnalysis(const std::string& output_var, const std::string& system, double delta);
    std::string generateReport(const std::string& system) const;
    bool validateConsistency(const std::string& system) const;
...
namespace saved_states_buoyancy {
...
std::string UQFFBuoyancyModule::getSystemName() const {
    return "UQFFBuoyancy_MultiSystem_UQFF";
}

// Batch Operations
void UQFFBuoyancyModule::transformVariableGroup(const std::vector<std::string>& names, 
...
void UQFFBuoyancyModule::scaleVariableGroup(const std::vector<std::string>& names, cdouble scale_factor) {
...
void UQFFBuoyancyModule::expandParameterSpace(double expansion_factor) {
    // Scale key exploration parameters across all systems
    std::vector<std::string> explore_params = {"k_LENR", "k_act", "k_DE", "k_neutron", "k_rel", "F_rel"};
...
void UQFFBuoyancyModule::expandSystemScale(const std::string& system, double mass_factor, double radius_factor) {
    // Expand observational system scale: mass and radius
    setSystemParams(system);
    
    variables["M"] *= cdouble(mass_factor, 0.0);
    variables["r"] *= cdouble(radius_factor, 0.0);
    
    // Adjust dependent parameters based on system type
    if (system == "ESO137" || system == "NGC1365" || system == "ElGordo") {
        // Galaxy/cluster systems: L_X scales with mass and radius
        if (variables.find("L_X") != variables.end()) {
            variables["L_X"] *= cdouble(std::pow(mass_factor, 1.5) / radius_factor, 0.0);
        }
        // Gas density scales inversely with volume
        ...
        // Pulsar: B field scales with mass/radius^3
        if (variables.find("B0") != variables.end()) {
            variables["B0"] *= cdouble(mass_factor / (radius_factor * radius_factor * radius_factor), 0.0);
        }
        // Spin frequency scales with sqrt(M/r^3)
        if (variables.find("omega0") != variables.end()) {
            variables["omega0"] *= cdouble(std::sqrt(mass_factor / (radius_factor * radius_factor * radius_factor)), 0.0);
        }
    } else if (system == "ASASSN14li") {
        // TDE: Accretion rate and luminosity scale with M/r
        if (variables.find("L_X") != variables.end()) {
            variables["L_X"] *= cdouble(mass_factor / radius_factor, 0.0);
        }
    }
}

void UQFFBuoyancyModule::expandForceScale(double dpm_factor, double lenr_factor) {
...
void UQFFBuoyancyModule::expandBuoyancyScale(const std::string& system, double buoyancy_factor, double vacuum_factor) {
    // Expand buoyancy-specific features
    setSystemParams(system);
    
    variables["beta_i"] *= cdouble(buoyancy_factor, 0.0);
    variables["V_infl_UA"] *= cdouble(buoyancy_factor, 0.0);
    variables["rho_vac_A"] *= cdouble(vacuum_factor, 0.0);
    variables["rho_vac_UA"] *= cdouble(vacuum_factor, 0.0);
    variables["rho_vac_SCm"] *= cdouble(vacuum_factor, 0.0);
    
    // Universal acceleration scales with buoyancy
    variables["a_universal"] *= cdouble(buoyancy_factor, 0.0);
}

// Self-Refinement
void UQFFBuoyancyModule::autoRefineParameters(double tolerance) {
    // Iteratively adjust parameters to minimize force residual
    for (int iter = 0; iter < 100; ++iter) {
        cdouble F_current = computeFBi("ESO137", variables["t"].real());  // Use default system
        ...
        variables["F_rel"] *= cdouble(0.999, 0.0);
    }
}

void UQFFBuoyancyModule::calibrateToObservations(const std::map<std::string, cdouble>& observations) {
...
void UQFFBuoyancyModule::optimizeForMetric(const std::string& metric, const std::string& system) {
    // Optimize parameters for specific metrics and systems
    setSystemParams(system);
    
    if (metric == "standard") {
        // Standard parameters for each system
        setSystemParams(system);  // Reset to defaults
        
    } else if (metric == "high_luminosity") {
        // Enhanced X-ray emission state
        variables["L_X"] *= cdouble(5.0, 0.0);
        variables["k_DE"] *= cdouble(10.0, 0.0);
        variables["F_rel"] *= cdouble(2.0, 0.0);
        
    } else if (metric == "strong_magnetic") {
        // Enhanced magnetic field state
        variables["B0"] *= cdouble(3.0, 0.0);
        variables["k_neutron"] *= cdouble(2.0, 0.0);
        
    } else if (metric == "buoyancy_dominant") {
        // Enhanced buoyancy forces
        variables["beta_i"] *= cdouble(2.0, 0.0);
        variables["a_universal"] *= cdouble(1.5, 0.0);
        variables["V_infl_UA"] *= cdouble(1.5, 0.0);
        
    } else if (metric == "relativistic") {
        // Enhanced relativistic effects
        variables["F_rel"] *= cdouble(3.0, 0.0);
        variables["k_rel"] *= cdouble(5.0, 0.0);
        
    } else if (metric == "lenr_dominant") {
        // LENR-dominated regime
        variables["k_LENR"] *= cdouble(10.0, 0.0);
        variables["omega_LENR"] *= cdouble(2.0, 0.0);
    }
}
...
void UQFFBuoyancyModule::evolveSystem(int generations, std::function<double(const UQFFBuoyancyModule&)> fitness) {
...
void UQFFBuoyancyModule::saveState(const std::string& state_name) {
    saved_states_buoyancy::states[state_name] = variables;
}

void UQFFBuoyancyModule::restoreState(const std::string& state_name) {
    if (saved_states_buoyancy::states.find(state_name) != saved_states_buoyancy::states.end()) {
        variables = saved_states_buoyancy::states[state_name];
    }
}

std::vector<std::string> UQFFBuoyancyModule::listSavedStates() const {
    std::vector<std::string> names;
    for (const auto& pair : saved_states_buoyancy::states) {
        names.push_back(pair.first);
    }
    return names;
}

std::string UQFFBuoyancyModule::exportStateUQFF() const {
...
// System Analysis
std::map<std::string, double> UQFFBuoyancyModule::sensitivityAnalysis(const std::string& output_var, const std::string& system, double delta) {
    std::map<std::string, double> sensitivities;
    setSystemParams(system);
    cdouble baseline = computeFBi(system, variables["t"].real());
    
    for (auto& pair : variables) {
        cdouble original = pair.second;
        pair.second = original * cdouble(1.0 + delta, 1.0);
        cdouble perturbed = computeFBi(system, variables["t"].real());
        
        double sensitivity = std::abs(perturbed - baseline) / (std::abs(baseline) + 1e-200);
...
std::string UQFFBuoyancyModule::generateReport(const std::string& system) const {
    ...
    report << "=== UQFF Buoyancy Multi-System Report ===\n";
    report << "System: " << getSystemName() << "\n";
    report << "Active System: " << system << "\n";
    report << "Total Variables: " << variables.size() << "\n";
    report << "\nKey Parameters:\n";
    report << "  M (Mass) = " << std::scientific << variables.at("M").real() << " kg\n";
    ...
    report << "  rho_gas (Gas Density) = " << variables.at("rho_gas").real() << " kg/m^3\n";
    report << "  F_rel (Relativistic Coherence) = " << variables.at("F_rel").real() << " N\n";
    report << "\nBuoyancy Parameters:\n";
    report << "  beta_i = " << variables.at("beta_i").real() << "\n";
    report << "  V_infl_UA = " << variables.at("V_infl_UA").real() << " + i " << variables.at("V_infl_UA").imag() << "\n";
    report << "  a_universal = " << variables.at("a_universal").real() << " + i " << variables.at("a_universal").imag() << "\n";
    report << "\nSupported Systems:\n";
    report << "  - ESO 137-001: Ram pressure stripping galaxy (M=2e41 kg, r=6.17e21 m)\n";
    report << "  - NGC 1365: Barred spiral with AGN (M=7.17e41 kg, r=9.46e20 m)\n";
    report << "  - Vela Pulsar: Young neutron star (M=2.8e30 kg, r=1.7e17 m)\n";
    report << "  - ASASSN-14li: Tidal disruption event (M=1.989e37 kg, r=3.09e18 m)\n";
    report << "  - El Gordo: Massive galaxy cluster (M=4.97e45 kg, r=3.09e22 m)\n";
    report << "\nPhysics: Multi-system buoyancy with Sweet/Kozima, F_rel=4.30e33 N\n";
    report << "Validation: Chandra, JWST, ALMA observational data\n";
    
    return report.str();
}

bool UQFFBuoyancyModule::validateConsistency(const std::string& system) const {
    // Check physical constraints for the specified system
    ...
    if (system == "ESO137" || system == "NGC1365") {
        // Galaxy scale
        if (M_val < 1e40 || M_val > 1e43) valid = false;
        if (r_val < 1e19 || r_val > 1e23) valid = false;
        if (L_X_val < 1e32 || L_X_val > 1e38) valid = false;
        if (B_val < 1e-12 || B_val > 1e-7) valid = false;
        
    } else if (system == "Vela") {
        // Pulsar scale
        if (M_val < 1e30 || M_val > 5e30) valid = false;
        if (r_val < 1e16 || r_val > 1e19) valid = false;
        if (B_val < 1e-10 || B_val > 1e-6) valid = false;
        
    } else if (system == "ASASSN14li") {
        // TDE scale
        if (M_val < 1e36 || M_val > 1e39) valid = false;
        if (r_val < 1e17 || r_val > 1e20) valid = false;
        if (L_X_val < 1e35 || L_X_val > 1e39) valid = false;
        
    } else if (system == "ElGordo") {
        // Massive cluster scale
        if (M_val < 1e44 || M_val > 1e47) valid = false;
        if (r_val < 1e21 || r_val > 1e24) valid = false;
        if (L_X_val < 1e36 || L_X_val > 1e40) valid = false;
    }
...
void UQFFBuoyancyModule::autoCorrectAnomalies(const std::string& system) {
    // Clamp parameters to physically reasonable ranges for each system
    double M_val = variables["M"].real();
    double r_val = variables["r"].real();
    ...
    if (system == "ESO137" || system == "NGC1365") {
        // Galaxy scale corrections
        if (M_val < 1e40) variables["M"] = {1e40, 0.0};
        if (M_val > 1e43) variables["M"] = {1e43, 0.0};
        if (r_val < 1e19) variables["r"] = {1e19, 0.0};
        if (r_val > 1e23) variables["r"] = {1e23, 0.0};
        if (L_X_val < 1e32) variables["L_X"] = {1e32, 0.0};
        if (L_X_val > 1e38) variables["L_X"] = {1e38, 0.0};
        if (B_val < 1e-12) variables["B0"] = {1e-12, 0.0};
        if (B_val > 1e-7) variables["B0"] = {1e-7, 0.0};
        
    } else if (system == "Vela") {
        // Pulsar scale corrections
...
/*
=============================================================================
COMPREHENSIVE DEMONSTRATION: UQFF Buoyancy Multi-System Dynamic Module Enhancement
=============================================================================

#include "source155.cpp"  // or link against compiled module

int main() {
    std::cout << "=== UQFF Buoyancy Multi-System Dynamic Enhancement Test ===" << std::endl;
    
    // Initialize module with default parameters
    UQFFBuoyancyModule buoyancy;
    
    // === Test 1: Variable Management ===
    std::cout << "\n--- Test 1: Variable Management ---" << std::endl;
    buoyancy.createVariable("test_buoyancy_param", {1e-20, 5e-21});
    buoyancy.cloneVariable("F_rel", "F_rel_backup");
    std::vector<std::string> vars = buoyancy.listVariables();
    std::cout << "Total variables: " << vars.size() << std::endl;
    std::cout << "System Name: " << buoyancy.getSystemName() << std::endl;
    
    // === Test 2: Batch Operations ===
    std::cout << "\n--- Test 2: Batch Operations ---" << std::endl;
    std::vector<std::string> force_params = {"k_LENR", "k_act", "k_DE", "F_rel"};
    buoyancy.scaleVariableGroup(force_params, {1.5, 0.0});
    std::cout << "Scaled force parameters by 1.5x" << std::endl;
    
    // === Test 3: Self-Expansion (System Scale - ESO 137-001) ===
    std::cout << "\n--- Test 3: System Scale Expansion (ESO137) ---" << std::endl;
    buoyancy.saveState("before_eso_expansion");
    buoyancy.expandSystemScale("ESO137", 1.2, 1.3);  // 20% more mass, 30% larger radius
    std::cout << "Expanded ESO137 scale: M×1.2, r×1.3" << std::endl;
    buoyancy.restoreState("before_eso_expansion");
    
    // === Test 4: Self-Expansion (Force Scale) ===
    std::cout << "\n--- Test 4: Force Scale Expansion ---" << std::endl;
    buoyancy.expandForceScale(2.0, 1.5);  // Double DPM, increase LENR by 50%
    std::cout << "Expanded force coupling: DPM×2.0, LENR×1.5" << std::endl;
    
    // === Test 5: Self-Expansion (Buoyancy Scale - NGC 1365) ===
    std::cout << "\n--- Test 5: Buoyancy Scale Expansion (NGC1365) ---" << std::endl;
    buoyancy.saveState("before_buoyancy_expansion");
    buoyancy.expandBuoyancyScale("NGC1365", 1.5, 1.3);  // 50% more buoyancy, 30% more vacuum
    std::cout << "Expanded NGC1365 buoyancy scale: β×1.5, ρ_vac×1.3" << std::endl;
    buoyancy.restoreState("before_buoyancy_expansion");
    
    // === Test 6: Parameter Space Expansion ===
    std::cout << "\n--- Test 6: Parameter Space Expansion ---" << std::endl;
    buoyancy.expandParameterSpace(1.25);
    std::cout << "Expanded exploration parameters by 25%" << std::endl;
    
    // === Test 7: Auto-Refinement ===
    std::cout << "\n--- Test 7: Auto-Refinement ---" << std::endl;
    buoyancy.autoRefineParameters(1e200);
    std::cout << "Auto-refined parameters to minimize force residual" << std::endl;
    
    // === Test 8: Calibration to Observations ===
    std::cout << "\n--- Test 8: Calibration to Observational Data ---" << std::endl;
    std::map<std::string, cdouble> observations = {
        {"F_rel", {5e33, 0.0}},
        {"beta_i", {0.7, 0.0}},
        {"k_LENR", {2e-10, 0.0}}
    };
    buoyancy.calibrateToObservations(observations);
    std::cout << "Calibrated to observational data" << std::endl;
    
    // === Test 9: Optimization for Metrics (Vela Pulsar) ===
    std::cout << "\n--- Test 9: Optimization for Standard Vela ---" << std::endl;
    buoyancy.saveState("before_optimization");
    buoyancy.optimizeForMetric("standard", "Vela");
    std::cout << "Optimized for standard Vela pulsar state" << std::endl;
    
    std::cout << "\n--- Test 10: Optimization for High Luminosity (ASASSN14li) ---" << std::endl;
    buoyancy.optimizeForMetric("high_luminosity", "ASASSN14li");
    std::cout << "Optimized for enhanced X-ray emission (TDE)" << std::endl;
    std::cout << "L_X = " << buoyancy.variables["L_X"].real() << " W" << std::endl;
    
    std::cout << "\n--- Test 11: Optimization for Strong Magnetic (Vela) ---" << std::endl;
    buoyancy.optimizeForMetric("strong_magnetic", "Vela");
    std::cout << "Optimized for enhanced magnetic field state" << std::endl;
    std::cout << "B0 = " << buoyancy.variables["B0"].real() << " T" << std::endl;
    
    buoyancy.restoreState("before_optimization");
    
    // === Test 12: Parameter Variations ===
    std::cout << "\n--- Test 12: Parameter Variations ---" << std::endl;
    auto variations = buoyancy.generateVariations(5, 0.1);
    std::cout << "Generated " << variations.size() << " parameter variations (±10%)" << std::endl;
    
    // === Test 13: Mutation ===
    std::cout << "\n--- Test 13: Parameter Mutation ---" << std::endl;
    buoyancy.saveState("before_mutation");
    buoyancy.mutateParameters(0.05);
    std::cout << "Applied 5% mutation to all parameters" << std::endl;
    buoyancy.restoreState("before_mutation");
    
    // === Test 14: Evolutionary Optimization ===
    std::cout << "\n--- Test 14: Evolutionary Optimization ---" << std::endl;
    auto fitness = [](const UQFFBuoyancyModule& m) -> double {
        double F_mag = std::abs(m.computeFBi("ElGordo", 1e12));
        return 1.0 / (1.0 + F_mag / 1e210);  // Minimize force magnitude
    };
    buoyancy.evolveSystem(10, fitness);
    std::cout << "Evolved system over 10 generations" << std::endl;
    
    // === Test 15: State Management ===
    std::cout << "\n--- Test 15: State Management ---" << std::endl;
    buoyancy.saveState("final_buoyancy_state");
    std::vector<std::string> states = buoyancy.listSavedStates();
    ...
    std::string exported = buoyancy.exportStateUQFF();
    std::cout << "Exported state (first 200 chars): " << exported.substr(0, 200) << "..." << std::endl;
    
    // === Test 17: Sensitivity Analysis (El Gordo) ===
    std::cout << "\n--- Test 17: Sensitivity Analysis (ElGordo) ---" << std::endl;
    auto sensitivities = buoyancy.sensitivityAnalysis("F", "ElGordo", 0.01);
    std::cout << "Top 5 sensitive parameters for El Gordo:" << std::endl;
    ...
    // === Test 18: System Reports (All Systems) ===
    std::cout << "\n--- Test 18: System Reports ---" << std::endl;
    std::vector<std::string> systems = {"ESO137", "NGC1365", "Vela", "ASASSN14li", "ElGordo"};
    for (const auto& sys : systems) {
        std::cout << "\n" << buoyancy.generateReport(sys) << std::endl;
    }
    
    // === Test 19: Consistency Validation (All Systems) ===
    std::cout << "\n--- Test 19: Consistency Validation ---" << std::endl;
    for (const auto& sys : systems) {
        buoyancy.setSystemParams(sys);
        bool is_valid = buoyancy.validateConsistency(sys);
        std::cout << sys << " consistency: " << (is_valid ? "VALID" : "INVALID") << std::endl;
    }
    
    // === Test 20: Anomaly Correction (NGC1365) ===
    std::cout << "\n--- Test 20: Anomaly Auto-Correction ---" << std::endl;
    buoyancy.setSystemParams("NGC1365");
    buoyancy.variables["M"] = {1e50, 0.0};  // Unphysical high mass
    std::cout << "Injected anomaly in NGC1365: M = " << buoyancy.variables["M"].real() << " kg" << std::endl;
    buoyancy.autoCorrectAnomalies("NGC1365");
    std::cout << "Auto-corrected M = " << buoyancy.variables["M"].real() << " kg" << std::endl;
    
    // === Final Force Computations (All Systems) ===
    std::cout << "\n=== Final Force Computations ===" << std::endl;
    for (const auto& sys : systems) {
        double t_test = 1e12;  // 1e12 seconds
        cdouble F_final = buoyancy.computeFBi(sys, t_test);
        std::cout << sys << " F_U(t=" << t_test << " s) = " << std::scientific << std::setprecision(6)
    ...
    std::cout << "UQFF Buoyancy module now supports:" << std::endl;
    std::cout << "  ✓ Variable management (create, remove, clone, list)" << std::endl;
    std::cout << "  ✓ Batch operations (transform, scale groups)" << std::endl;
    std::cout << "  ✓ Self-expansion (system scale, force scale, buoyancy scale)" << std::endl;
    ...
    std::cout << "  ✓ Multi-system support (ESO137, NGC1365, Vela, ASASSN14li, ElGordo)" << std::endl;
