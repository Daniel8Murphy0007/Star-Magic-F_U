...
std::string UniversalInertiaVacuumModule::getSystemName() const {
    return "Universal_Inertia_Vacuum_rho_vac_Ui_UQFF";
}
...
std::string UniversalInertiaVacuumModule::exportState() {
    ...
    oss << "UniversalInertiaVacuumModule State:\n";
    for (const auto& pair : variables) {
...
std::string UniversalInertiaVacuumModule::generateReport() {
    ...
    oss << "========== Universal Inertia Vacuum Module Report ==========\n";
...
int main() {
    ...
    std::cout << "========== UNIVERSAL INERTIA VACUUM MODULE DEMONSTRATION ==========\n\n";
    ...
    mod.createVariable("inertia_force_scale", -u_i * 1e10);  // F_U contribution scale
    std::cout << "  Created 'inertia_force_scale' = " << mod.variables["inertia_force_scale"] << "\n";
    
    std::vector<std::string> all_vars = mod.listVariables();
    std::cout << "  Total variables: " << all_vars.size() << "\n";
    
    mod.cloneVariable("lambda_i", "lambda_i_initial");
    std::cout << "  Cloned 'lambda_i' → 'lambda_i_initial'\n\n";
    
    // ===== Step 4: Inertia Expansion (Higher Resistance) =====
    std::cout << "Step 4: Inertia Expansion (Increase Resistance)\n";
    mod.expandInertiaScale(2.0, 1.5);  // 2x λ_i coupling, 1.5x vacuum densities
    std::cout << "  After expansion:\n";
    std::cout << "    λ_i = " << mod.variables["lambda_i"] << "\n";
    std::cout << "    ρ_vac,Ui = " << mod.computeRho_vac_Ui() << " J/m³\n";
    std::cout << "    U_i = " << mod.computeU_i(0.0, 0.0) << " J/m³\n\n";
    
    // ===== Step 5: Vacuum Expansion (Density Increase) =====
    std::cout << "Step 5: Vacuum Expansion (Increase Vacuum Densities)\n";
    mod.expandVacuumScale(2.0, 3.0);  // 2x densities, 3x product
    std::cout << "  After expansion:\n";
    std::cout << "    ρ_vac,SCm = " << mod.variables["rho_vac_SCm"] << " J/m³\n";
    std::cout << "    ρ_vac,UA = " << mod.variables["rho_vac_UA"] << " J/m³\n";
    std::cout << "    Product = " << mod.variables["rho_product"] << " J²/m⁶\n";
    std::cout << "    U_i = " << mod.computeU_i(0.0, 0.0) << " J/m³\n\n";
    
    // ===== Step 6: Temporal Expansion (Rotation & Phase) =====
    std::cout << "Step 6: Temporal Expansion\n";
    mod.expandTemporalScale(1.5, 1.2);  // 1.5x ω_s, 1.2x t_n phase
    std::cout << "  After temporal expansion:\n";
    std::cout << "    ω_s = " << mod.variables["omega_s"] << " rad/s\n";
    std::cout << "    f_TRZ = " << mod.variables["f_TRZ"] << "\n";
    std::cout << "    U_i = " << mod.computeU_i(0.0, 0.0) << " J/m³\n\n";
    
    // ===== Step 7: Batch Operations (Reset Inertia Group) =====
    std::cout << "Step 7: Batch Operations (Scale Inertia Parameters)\n";
    std::vector<std::string> inertia_group = {"lambda_i", "rho_vac_Ui", "f_TRZ"};
    mod.scaleVariableGroup(inertia_group, 0.5);  // Reduce to low inertia
    ...
    // Low inertia (interstellar medium)
    mod.optimizeForMetric("low_inertia");
    std::cout << "  Low Inertia (ISM):\n";
    std::cout << "    ρ_vac,Ui = " << mod.computeRho_vac_Ui() << " J/m³\n";
    std::cout << "    U_i = " << mod.computeU_i(0.0, 0.0) << " J/m³\n";
    
    // Solar inertia (Sun reference)
    mod.optimizeForMetric("solar_inertia");
    std::cout << "  Solar Inertia (Sun):\n";
    std::cout << "    ρ_vac,Ui = " << mod.computeRho_vac_Ui() << " J/m³\n";
    std::cout << "    U_i = " << mod.computeU_i(0.0, 0.0) << " J/m³\n";
    
    // Nebula inertia (star-forming regions)
    mod.optimizeForMetric("nebula_inertia");
    std::cout << "  Nebula Inertia (Star Formation):\n";
    std::cout << "    ρ_vac,Ui = " << mod.computeRho_vac_Ui() << " J/m³\n";
    std::cout << "    U_i = " << mod.computeU_i(0.0, 0.0) << " J/m³\n";
    
    // Galaxy core (high rotation)
    mod.optimizeForMetric("galaxy_core");
    std::cout << "  Galaxy Core:\n";
    std::cout << "    ρ_vac,Ui = " << mod.computeRho_vac_Ui() << " J/m³\n";
    std::cout << "    ω_s = " << mod.variables["omega_s"] << " rad/s\n";
    std::cout << "    U_i = " << mod.computeU_i(0.0, 0.0) << " J/m³\n";
    
    // High resistance (quasar jets)
    mod.optimizeForMetric("high_resistance");
    std::cout << "  High Resistance (Quasar Jets):\n";
    std::cout << "    ρ_vac,Ui = " << mod.computeRho_vac_Ui() << " J/m³\n";
    std::cout << "    λ_i = " << mod.variables["lambda_i"] << "\n";
    std::cout << "    U_i = " << mod.computeU_i(0.0, 0.0) << " J/m³\n\n";
    ...
    obs_data["rho_vac_Ui"] = 3.0e-36;  // Observed value
    obs_data["lambda_i"] = 1.2;
    obs_data["omega_s"] = 2.8e-6;
    ...
    for (size_t i = 0; i < std::min(size_t(3), variations.size()); ++i) {
        std::cout << "    Variant " << (i+1) << ": ρ_vac,Ui=" << variations[i]["rho_vac_Ui"] 
                  << " J/m³, λ_i=" << variations[i]["lambda_i"] << "\n";
    }
    std::cout << "\n";
    
    // ===== Step 16: Mutation =====
    std::cout << "Step 16: Mutate Parameters\n";
    mod.optimizeForMetric("solar_inertia");  // Reset to solar
    mod.mutateParameters(0.15);  // 15% mutation
    std::cout << "  After mutation: ρ_vac,Ui = " << mod.computeRho_vac_Ui() << " J/m³\n";
    std::cout << "  After mutation: λ_i = " << mod.variables["lambda_i"] << "\n";
    std::cout << "  After mutation: ω_s = " << mod.variables["omega_s"] << " rad/s\n\n";
    
    // ===== Step 17: System Evolution =====
    std::cout << "Step 17: Evolve System (Minimize Inertial Resistance)\n";
    mod.evolveSystem(10);  // 10 generations
    std::cout << "  After evolution: ρ_vac,Ui = " << mod.computeRho_vac_Ui() << " J/m³\n";
    std::cout << "  After evolution: λ_i = " << mod.variables["lambda_i"] << "\n";
    std::cout << "  After evolution: U_i = " << mod.computeU_i(0.0, 0.0) << " J/m³\n\n";
    ...
    std::cout << "  (Includes all vacuum densities, inertia parameters, temporal factors)\n\n";
    ...
    // ===== Step 23-26: Inertia Scale Analysis =====
