/**
 * ================================================================================================
 * Module: source17.js (Westerlund2)
 *
 * Description: JavaScript Module for Westerlund 2 Super Star Cluster Class
 ...
 *              Usage: import Westerlund2 from './source17.js';
 *                     const wd2 = new Westerlund2();
 *                     const g = wd2.compute_g_Westerlund2(t);
 *
 * Key Features:
 *   - Default values from UQFF document: M_initial = 30,000 Msun, r = 10 ly, B = 1e-5 T,
 *     M_dot_factor ≈ 3.333, tau_SF = 2 Myr, rho_wind = 1e-20 kg/m^3, v_wind = 2e6 m/s.
 ...
 *
 * Converted from: source17.cpp (C++ module)
...
class Westerlund2 {
    ...
    /**
     * Initialize all parameters with default UQFF values for Westerlund 2.
     */
    initializeDefaults() {
        this.G = 6.6743e-11;
        const M_sun = 1.989e30;
        const M_initial_sun = 30000.0;
        this.M_initial = M_initial_sun * M_sun;
        this.r = 9.461e16;  // 10 ly
        this.H0 = 2.184e-18;
        this.B = 1e-5;
        this.B_crit = 1e11;
        this.Lambda = 1.1e-52;
        ...
        this.gas_v = 1e5;
        this.f_TRZ = 0.1;
        this.M_dot_factor = 1e5 / M_initial_sun;  // ~3.333
        this.tau_SF = 2e6 * 3.156e7;
        this.rho_wind = 1e-20;
        this.v_wind = 2e6;
        ...
        this.A_osc = 1e-9;  // Adjusted for cluster scale
    ...
    /**
     * Compute volume of the star cluster (spherical approximation).
    ...
    /**
     * Main MUGE computation for Westerlund 2 (includes ALL terms).
     * Returns total gravitational acceleration at distance r and time t.
     * @param {number} t - Time in seconds.
     * @returns {number} - Total acceleration g_Westerlund2 (m/s^2).
     */
    ...
    printParameters(stream = console) {
        stream.log('Westerlund 2 Parameters:');
    ...
    /**
     * Example computation at t=1 Myr (for quick testing).
     * @returns {number} - Acceleration at t=1 Myr (m/s^2).
     */
    exampleAt1Myr() {
        const t_example = 1e6 * 3.156e7;
     ...
     */
    removeVariable(name) {
        console.error(`Warning: Cannot remove built-in variable '${name}' in Westerlund2 class.`);
        return false;
    }
    ...
    autoRefineParameters(observations) {
        ...
        for (const obs of observations) {
            const t = obs[0];
            const g_obs = obs[1];
            const g_calc = this.compute_g_Westerlund2(t);
    ...
    optimizeForMetric(metric, t_start, t_end, steps) {
        ...
        for (let i = 0; i <= steps; i++) {
            ...
            const g = this.compute_g_Westerlund2(t);
    ...
    saveState(stateName) {
        ...
        Westerlund2.savedStates[stateName] = state;
        return true;
    }
    ...
    restoreState(stateName) {
        const state = Westerlund2.savedStates[stateName];
     ...
     */
    listSavedStates() {
        return Object.keys(Westerlund2.savedStates);
    }
     ...
     */
    exportState() {
        let output = 'Westerlund2 State Export:\n';
        const vars = this.listVariables();
        for (const v of vars) {
    ...
    sensitivityAnalysis(t, delta_pct) {
        const sensitivities = {};
        const g_base = this.compute_g_Westerlund2(t);
        ...
        for (const v of vars) {
            ...
            const g_plus = this.compute_g_Westerlund2(t);
    ...
    generateReport(t) {
        let output = '';
        output += '============================================\n';
        output += 'WESTERLUND 2 SUPER STAR CLUSTER REPORT\n';
        output += '============================================\n';
        ...
        output += `  g_Westerlund2(t) = ${this.compute_g_Westerlund2(t).toExponential(6)} m/s^2\n\n`;
    ...
    autoCorrectAnomalies() {
        ...
        if (this.M_initial <= 0) { this.M_initial = 30000.0 * M_sun; corrected = true; }
        if (this.r <= 0) { this.r = 9.461e16; corrected = true; }
        if (this.B < 0) { this.B = 1e-5; corrected = true; }
        if (this.B_crit <= 0) { this.B_crit = 1e11; corrected = true; }
        if (this.tau_SF <= 0) { this.tau_SF = 2e6 * 3.156e7; corrected = true; }
...
/**
 * Enhanced example demonstrating all Westerlund2 capabilities.
 */
function enhancedWesterlund2Example() {
    ...
    wd2.printParameters();
    const t0 = 0.0;
    console.log(`g_Westerlund2(t=0) = ${wd2.compute_g_Westerlund2(t0).toExponential(6)} m/s^2\n`);

    // Step 2: Time evolution (0, 0.5, 1, 2, 5 Myr)
    console.log('Step 2: Time Evolution (0, 0.5, 1, 2, 5 Myr)');
    const M_sun = 1.989e30;
    for (const t_myr of [0.0, 0.5, 1.0, 2.0, 5.0]) {
        const t = t_myr * 1e6 * 3.156e7;
        console.log(`  t = ${t_myr} Myr: g = ${wd2.compute_g_Westerlund2(t).toExponential(6)} m/s^2, M(t) = ${(wd2.M_t(t)/M_sun).toFixed(2)} M_sun`);
    }
    console.log();

    // Step 3: Star formation scaling
    console.log('Step 3: Star Formation Scaling (M_dot_factor x1.4, tau_SF x0.75)');
    wd2.expandStarFormationScale(1.4, 0.75);
    const t_test = 1e6 * 3.156e7;
    console.log(`After expansion: M_dot_factor = ${wd2.getVariable('M_dot_factor').toFixed(3)}, tau_SF = ${wd2.getVariable('tau_SF').toExponential(3)} s`);
    console.log(`g_Westerlund2(t=1 Myr) = ${wd2.compute_g_Westerlund2(t_test).toExponential(6)} m/s^2\n`);

    // Step 4: Wind feedback scaling
    console.log('Step 4: Wind Feedback Scaling (rho_wind x1.6, v_wind x1.25)');
    wd2.expandWindFeedbackScale(1.6, 1.25);
    console.log(`After expansion: rho_wind = ${wd2.getVariable('rho_wind').toExponential(6)} kg/m^3, v_wind = ${wd2.getVariable('v_wind').toExponential(3)} m/s`);
    console.log(`g_Westerlund2(t=1 Myr) = ${wd2.compute_g_Westerlund2(t_test).toExponential(6)} m/s^2\n`);

    // Step 5: Magnetic & fluid scaling
    console.log('Step 5: Magnetic & Fluid Scaling (B x1.3, rho_fluid x1.4)');
    wd2.expandMagneticFluidScale(1.3, 1.4);
    console.log(`After expansion: B = ${wd2.getVariable('B').toExponential(6)} T, rho_fluid = ${wd2.getVariable('rho_fluid').toExponential(6)} kg/m^3`);
    console.log(`g_Westerlund2(t=1 Myr) = ${wd2.compute_g_Westerlund2(t_test).toExponential(6)} m/s^2\n`);
    ...
    console.log(`Modified M_initial to 50000 M_sun: g = ${wd2.compute_g_Westerlund2(t_test).toExponential(6)} m/s^2`);
    wd2.restoreState('expanded_state');
    console.log(`Restored state: M_initial = ${(wd2.getVariable('M_initial')/M_sun).toFixed(2)} M_sun, g = ${wd2.compute_g_Westerlund2(t_test).toExponential(6)} m/s^2`);
    console.log(`Saved states: ${wd2.listSavedStates().join(' ')}\n`);

    // Step 7: Sensitivity analysis
    console.log('Step 7: Sensitivity Analysis at t=1 Myr (top 5 parameters)');
    ...
    const variations = wd2.generateVariations(3, 10.0);
    ...
    console.log('Step 9: Batch Transform (scale density parameters by 1.25)');
    wd2.transformVariableGroup(['rho_fluid', 'rho_wind'], v => v * 1.25);
    console.log(`After transform: rho_fluid = ${wd2.getVariable('rho_fluid').toExponential(6)} kg/m^3, rho_wind = ${wd2.getVariable('rho_wind').toExponential(6)} kg/m^3`);
    console.log(`g_Westerlund2(t=1 Myr) = ${wd2.compute_g_Westerlund2(t_test).toExponential(6)} m/s^2\n`);
    ...
    console.log('Step 11: Optimize for Maximum g (t=0 to 5 Myr, 100 steps)');
    const max_g = wd2.optimizeForMetric(g => g, 0.0, 5e6 * 3.156e7, 100);
    console.log(`Maximum g found: ${max_g.toExponential(6)} m/s^2\n`);

    // Step 12: Full system report
    console.log('Step 12: Full System Report at t=1.5 Myr');
...
export default Westerlund2;
export { enhancedWesterlund2Example };
